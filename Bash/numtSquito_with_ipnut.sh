...
FILE_in=($(basename $1 .bam))

INPUT_BAM=$FILE_in   #select only the first sample
...
# files are stored in discordant with the D flag
# files are stored in splitted with the e flag
# files that go to null actually are all the good reads of the mosquito that are neither taken as split or discordant...

echo "Running first samtools"
samtools view -@ 4 -m 10G -h -F 2  $INPUT_BAM | grep -e @ -e MT -e chrM | samtools sort -@ 4 -m 10G -n  | samtools view -h |  samblaster --ignoreUnmated -e -d $INPUT_DISC -s $INPUT_SPLIT -o /data/botos/2022_12_01_NUMTS_Aedes_Aegypti/null_${FILE_in}
echo "First samtools done"

echo -e "Running clustering of samples"
...
while read line; do
	...
	echo -e "perl 2 done"
        ...
        blat ${REF_Genome} ${OUTPUT}.fasta ${OUTPUT}.psl
	echo -e "blat done"
