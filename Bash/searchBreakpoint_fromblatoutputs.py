...
readLEN = 147 # wgs read length, changed to aedes aegypti
START = int(START)
END = int(END)
#not needed to change as there is not chr in aedes aegypti
...
#if review need to change to the comment line
df_nu = df2[ (df2['Tname'].isin([CHR])) & (df2['Tstart'] > START) & (df2['Tend'] < END)]
#df_nu = df2[ (df2['Tname'].isin([str(CHR)])) & (df2['Tstart'] > START) & (df2['Tend'] < END)]
df_m = pd.concat([df_nu, df_mt])
print("printing df_nu")
df_nu.head()
print(df_nu)
print("printing df_mt")
df_mt.head()
print(df_mt)
print(df_nu.apply(f_nu, axis=1))
print(df_mt.apply(f_mt, axis=1))
